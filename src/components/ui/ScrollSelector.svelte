<script lang="ts">
	...
	export let selectedScroll: string;
	export let onSelect: (scrollName: string) => void;
	...
	$: selectedScrollEntry = scrollList.find((scroll) => scroll.name === selectedScroll);
...
<div class="flex flex-col gap-2">
	...
	{#if selectedScrollEntry}
		<p class="text-sm text-gray-600">{selectedScrollEntry.description}</p>
	{/if}
</div>
